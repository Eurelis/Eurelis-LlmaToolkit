...
from eurelis_llmatoolkit.utils.base_factory import (
    JSON,
    DefaultFactories,
    ParamsDictFactory,
)

# from eurelis_llmatoolkit.langchain.acronyms.acronyms_document_transformer import (
#     AcronymsDocumentTransformer,
# ) # TODO Modifier / Implementer
from eurelis_llmatoolkit.llamaindex.dataset.dataset import Dataset
...
class DatasetFactory(ParamsDictFactory[Dataset]):
    ...
    def build(self, context: "BaseContext") -> Dataset:
        ...
        Args:
            context: the context object, usually the current indexation wrapper instance
        ...
        if not isinstance(context, IndexationWrapper):
            raise ValueError(
                "DatasetFactory should be used with IndexationWrapper instance as context"
            )

        # TODO : get_instance_from_factory
